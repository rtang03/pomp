...
export type TPrepareWriteConfig = ReturnType<typeof usePompPrepareWrite>['config'];

const SubmitForm: FC<{ setWagmiConfig: Dispatch<TPrepareWriteConfig> }> = ({ setWagmiConfig }) => {
  const { values, submitForm } = useFormikContext<{ verifier: string }>();
  ...
  useEffect(() => {
    config && setWagmiConfig(config);
  }, [config, setWagmiConfig]);
  return null;
};
...
const StartPrepared: FC<Props> = ({ missionDoc: m, setShow }) => {
  const { address } = useAccount();
  const { dev, isValidWalletAccount } = useAppContext();
  const [wagmiConfig, setWagmiConfig] = useState<TPrepareWriteConfig>();
  const { writeAsync, reset: writeAsyncReset } = useContractWrite(wagmiConfig);
            ...
            >
              {({ isSubmitting, status, isValid, values, setFieldValue, resetForm, setStatus }) => (
                <Form>
                  ...
                  <div className="my-20">
                    <SubmitForm setWagmiConfig={setWagmiConfig} />
                    <SaveButton
                      type="submit"
                      canSave={isValid}
